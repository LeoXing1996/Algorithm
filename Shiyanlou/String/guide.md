...
给出一个字符串序列, 根据其规律, 输入数字, 输出相应序号的字符串
...
* 输入&输出
    ...
    ```
    后一个字符串由前一个字符串 __某一种数字的个数__ 加上 __数字种类__ 组成. 例如第三个可以解释为:第二个字符串含有 1个2, 1个1, 即1211

### 解决方法

整个过程由递归实现, 通过递归找到n-1时的字符串, 之后通过对n-1时的字符串进行遍历与计数, 生成n时的字符串. 值得注意的是: 由于返回类型为字符串类型, 因此在计数后需要进行数字到字符的转化操作, 才可以将字符添进字符串中. 在`C`中, 将`int`转化为`char`的操作为: `char = int + '0'`.